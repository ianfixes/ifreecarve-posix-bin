...
# Assume photos from major events have been manually processed from DSLR.
# This script sprinkles phone pics into those directories as necessary
...
# TODO: need starting year + month as command line arg
...
def collate(dated_files)
  dated_files.each_with_object({}) do |f, acc|
    key = yield(D8.of_phone(f))
...
all_images, unrecognized = input_files.partition { |f| image_of_interest?(f) }
...
day_match_files, no_day_matches = uncopied_images.partition do |img| 
  output_dirs.count { |d| image_matches_directory_day(img, d) } > 0
end
...
single_month_match_files, multi_month_matches = month_match_files.partition do |img|
  output_dirs.count { |d| image_matches_directory_month(img, d) } == 1
end
...
collate_day = collate(single_day_match_files, &:to_s)

collate_day.each do |date_key, files|
  out_dir = output_dirs.find { |d| D8.of_dir(d) == D8.of_s(date_key) }
  interactively_archive(files, "exact unique day", out_dir)
end

collate_month = collate(single_month_match_files) { |d8| d8.month.to_s }
collate_month.each do |date_key, files|
  out_dir = output_dirs.find { |d| D8.of_dir(d) == D8.of_s(date_key) }
  interactively_archive(files, "exact unique month", out_dir)
end
